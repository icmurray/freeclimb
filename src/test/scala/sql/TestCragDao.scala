...
trait CragDaoSpec extends FunSpec
  ...
  /**
   * ActionRunners should be stateless, so this should be OK.
   *
   * Return an ActionRunner that will be used to run the actions generated by
   * the tests.  It's dependant upon the implementation of the Dao under test
   * because a database-backed DAO would setup the connection to the database,
   * whereas a fake Dao wouldn't.
   */
  def runner: ActionRunner

  /**
   * These provide the fixtures for the tests.
   *
   * They should construct the CragDao (and ClimbDao if necessary), and run
   * the given testCode.
   *
   * The reason for ``withCragAndClimbDaos`` is that fake CragDao and ClimbDao
   * are not independant of one another because of the way that updating a
   * climb should bump the revision of the associated Crag.  This means that
   * there needs to be a means of constructing both fake objects at the same
   * time, allowing for them to set themselves up to share some sort of state.
   */
  def withCragDao(testCode: CragDao => Any): Unit
  def withCragAndClimbDaos(testCode: (CragDao, ClimbDao) => Any): Unit

  def run[M[+_], A, I <: IsolationLevel](action: ApiAction[A, I])
  ...
  describe("Crag DAO") {
    ...
    describe("The delete action") {
      ...
      it("should fail if the crag has climbs associated with it") {
        withCragAndClimbDaos { (cragDao, climbDao) =>
          val result = run {
            for {
